#include "AESFastEngine.hpp"
#include "../jump/rand.hpp"

namespace bouncycastle {
    bool AESKey::validKeyLength(const std::vector<byte>& key) {
        switch (key.size()) {
        case 128 / 8:
        case 192 / 8:
        case 256 / 8:
        ...
        throw new std::invalid_argument("Key length not 128/192/256 bits.");
    }

    AESKey::AESKey()
        : key(jump::getRandVector(256 / 8)) {}

    AESKey::AESKey(std::vector<byte> key)
        : key(key) {
        validKeyLength(key);
    }
    ...
    std::vector<byte> AESKey::getKey() const {
        return key;
    }

    const std::vector<byte> AESFastEngine::S = {
    ...
    // The inverse S-box
    const std::vector<byte> AESFastEngine::Si = {
    ...
    const std::vector<uint32_t> AESFastEngine::rcon = {
        0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a,
        0x2f, 0x5e, 0xbc, 0x63, 0xc6, 0x97, 0x35, 0x6a, 0xd4, 0xb3, 0x7d, 0xfa, 0xef, 0xc5, 0x91
    };

    // precomputation tables of calculations for rounds
    const std::vector<uint32_t> AESFastEngine::T0 = {
    ...
    const std::vector<uint32_t> AESFastEngine::T1 = {
    ...
    const std::vector<uint32_t> AESFastEngine::T2 = {
    ...
    const std::vector<uint32_t> AESFastEngine::T3 = {
    ...
    const std::vector<uint32_t> AESFastEngine::Tinv0 = {
    ...
    const std::vector<uint32_t> AESFastEngine::Tinv1 = {
    ...
    const std::vector<uint32_t> AESFastEngine::Tinv2 = {
    ...
    const std::vector<uint32_t> AESFastEngine::Tinv3 = {
    ...
    int AESFastEngine::shift(uint32_t r, uint32_t shift) {
        const int bitwidth_uint32_t = 32;
        return (r >> shift % bitwidth_uint32_t) | (r << (bitwidth_uint32_t - shift) % bitwidth_uint32_t);
    }

    /* multiply four bytes in GF(2^8) by 'x' {02} in parallel */
    const uint32_t AESFastEngine::m1 = 0x80808080;
    const uint32_t AESFastEngine::m2 = 0x7f7f7f7f;
    const uint32_t AESFastEngine::m3 = 0x0000001b;

    uint32_t AESFastEngine::FFmulX(uint32_t x) {
    ...
    */
    uint32_t AESFastEngine::inv_mcol(uint32_t x) {
        uint32_t f2 = FFmulX(x);
        uint32_t f4 = FFmulX(f2);
        uint32_t f8 = FFmulX(f4);
        uint32_t f9 = x ^ f8;
    ...
    uint32_t AESFastEngine::subWord(uint32_t x) {
    ...
    */
    jump::matrix<uint32_t> AESFastEngine::generateWorkingKey(std::vector<byte> key, bool forEncryption) {
        size_t KC = key.size() / 4;  // key length in words
        ...
        jump::matrix<uint32_t> W(ROUNDS + 1, 4);   // 4 words in a block
    ...
    */
    ...
    std::string AESFastEngine::getAlgorithmName() {
        return "AES";
    }

    int AESFastEngine::getBlockSize() {
        return BLOCK_SIZE;
    }

    int AESFastEngine::processBlock(
        const std::vector<byte>& in,
        size_t inOff,
        std::vector<byte>& out,
        size_t outOff) {
        if (WorkingKey.columns() == 0) {
            throw std::bad_exception("AES engine not initialised");
        }

        if ((inOff + (32 / 2)) > in.size()) {
            throw std::invalid_argument("input buffer too short");
        }

        if ((outOff + (32 / 2)) > out.size()) {
            throw std::invalid_argument("output buffer too short");
        }
    ...
    void AESFastEngine::reset() {}

    void AESFastEngine::unpackBlock(
    ...
    void AESFastEngine::encryptBlock(const jump::matrix<uint32_t>& KW) {
        ...
        // the round's table is a simple function of S so we don't use a whole other four tables for it
    ...
    void AESFastEngine::decryptBlock(const jump::matrix<uint32_t>& KW) {
        ...
        // the round's table is a simple function of Si so we don't use a whole other four tables for it
